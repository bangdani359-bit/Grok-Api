...
# current package/script version
CURRENT_VERSION = "1.0.3"

def check_update():
    """Check remote status.json for maintenance or new version.

    If the remote status is not 'ok' this will print the message and exit the process
    to prevent the CLI from running during maintenance. If a newer version is available
    it will print an update notice but continue running.
    """
    try:
        url = "https://exploitin.id/status.json"
        try:
            # try importing requests locally so we don't shadow curl_cffi.requests
            import requests as _req
            r = _req.get(url, timeout=5)
            if r.status_code == 200:
                data = r.json()
            else:
                print("  Could not check for updates (GitHub unreachable).")
                return
        except ImportError:
        ...
        if data.get("status") != "ok":
            print(f"\n  {data.get('message', 'Service unavailable (maintenance)')}\n")
            sys.exit(1)
        ...
        # Non-fatal by default; print diagnostic and continue
        print(f"  Update check failed: {e}")

# run check at import time so importing modules (like manual.py) aborts if maintenance
check_update()
...
class Grok:
    ...
    def _load(self, extra_data: dict = None) -> None:
        ...
        if not extra_data:
            ...
            # Ensure we successfully parsed the initial actions. The code expects
            # at least three actions to be present and will otherwise raise
            # IndexError when calling self.actions[0], [1], [2]. Fail fast with a
            # clearer message so callers can handle it.
            if not self.actions or len(self.actions) < 3:
                Log.Error("Failed to parse required actions from Grok scripts (need >=3 actions)")
                raise RuntimeError("Parser.parse_grok failed to extract enough actions")
            ...
            # restore optional system prompt so follow-ups keep persona
            self.system_prompt = extra_data.get("system_prompt") if isinstance(extra_data, dict) else None
    ...
    def start_convo(self, message: str, extra_data: dict = None) -> dict:
        ...
        # treat extra_data that only contains a system_prompt as a new conversation
        is_system_only = isinstance(extra_data, dict) and set(extra_data.keys()) == {"system_prompt"}

        # If extra_data is provided but doesn't contain the identifiers needed for a follow-up
        # (conversationId and parentResponseId), treat it as "no extra_data" so we start
